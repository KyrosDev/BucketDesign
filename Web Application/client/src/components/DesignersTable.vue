<template>
  <div class="data-display">
    <header>
      <h1>Total Designers: {{ designers.length }}</h1>
      <input type="text" name="username" id="search-username" />
    </header>
    <table class="users-table">
      <thead>
        <tr>
          ...
          <th class="headVerified">Verified</th>
      ...
      <tbody>
        ...
        >
          ...
          <td class="username" @click="goProfile(designer.username)">
            {{ designer.username }}
          </td>
          <td class="contacts">{{ designer.email }}</td>
          <td class="verified">{{ designer.verified }}</td>
          <td class="createdAt">{{ designer.createdAt | luxon }}</td>
          <td class="actions">
            Warn -
    ...
    <transition name="fade" mode="in-out">
      <Modal
        ...
        :actions="[
          {
            name: 'Cancel, keep Designer',
            class: 'secondary',
            callback: secondary,
          },
          { name: 'Yes, delete Designer', class: 'primary', callback: primary },
        ]"
      />
    </transition>
  </div>
  ...
  methods: {
    ...
    sortByUsername() {
      if (!this.$data.isSorted) {
        ...
        this.$data.old = this.$data.designers;
